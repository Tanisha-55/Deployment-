...
class DeploymentAssistantProvider {
    ...
    private async getCopilotResponse(prompt: string): Promise<string> {
        try {
            ...
            if (models.length === 0) {
                this.log('No suitable Copilot models available. Available models:');
                
                // Try to get all models without filters to see what's available
                const allModels = await lm.selectChatModels({});
                this.log(`All available models: ${JSON.stringify(allModels.map(m => m.name))}`);
                
                throw new Error('No suitable Copilot models available. Please ensure you have GitHub Copilot enabled and configured.');
            }
